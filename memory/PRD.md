...
## Implemented (2026-09-24) — Windows 11 Desktop Production Hardening (self-tested, PASS)
- **Runtime server URL config** (independence from Emergent): `desktop/config-store.js` persists `apiUrl` in userData (`kintrack-config.json`, survives updates); IPC `config:get-api-url`/`config:set-api-url`/`config:test-api-url` (health-check); preload exposes `getApiUrl/setApiUrl/testApiUrl/openLogFolder/onOpenSettings`. Hardcoded Emergent preview URL removed from `desktop/main.js`.
- **Frontend runtime API base**: `frontend/src/lib/api.js` resolves backend URL at runtime (Electron config store in desktop, build-time env in web); axios baseURL set per-request. New overlay `ServerDown.jsx` ("Server tidak dapat dihubungi." + server URL + status + Coba Lagi + Pengaturan) on network failure. New public page `/pengaturan-server` (`ServerSettings.jsx`): edit/test/save API URL + Tentang Aplikasi (nama/versi/platform) + Buka Folder Log. Login page & topbar hide Windows-download link in desktop; "Server" nav item desktop-only. Electron menu Bantuan → Pengaturan Server / Tentang Aplikasi.
- **Installer config** (`desktop/package.json`): productName "Monitoring dan Pelaporan Kinerja PN Sukadana", NSIS `Sukadana-Kinerja-Setup-1.0.0.exe`, portable `Sukadana-Kinerja-1.0.0-Portable.exe`, shortcut "KINTRACK PN Sukadana"; scripts `dev`/`build` (build-renderer.js: yarn build frontend + copy renderer)/`dist:win`/`dist:win:installer`.
- Docs: `desktop/BUILD_WINDOWS.md` (full build/install/configure/update/troubleshoot guide), `desktop/config.example.json`. Security unchanged: contextIsolation+nodeIntegration off, no SIPP/DB creds in Electron (mysql2 LAN bridge kept as admin option).
- Verified: React production build OK; Electron launches serving bundled renderer (xvfb, logs correct API URL); prod-build login + overlay + settings navigation PASS. Windows .exe build NOT possible in this Linux env (wine/rcedit) — user builds on their Windows 11 per guide.
